...
serve(async (req) => {
  ...
  try {
    ...
    const activeOrTrialSubs = subscriptions.data.filter(sub => 
      sub.status === "active" || sub.status === "trialing"
    );
    
    const hasActiveSub = activeOrTrialSubs.length > 0;
    ...
    const { data: profileData } = await supabaseClient.from("profiles").select("organization_id").eq("user_id", user.id).single();
