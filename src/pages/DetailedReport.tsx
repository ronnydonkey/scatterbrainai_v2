...
const DetailedReport: React.FC = () => {
  ...
  const generateDetailedReport = useCallback(async (id: string) => {
    const result = await handleAsyncError(async () => {
      ...
      console.log('Calling detailed-report with data:', {
        insightId: id,
        originalInput: foundInsight.input || foundInsight.originalInput,
        basicInsights: foundInsight.response || foundInsight.insights,
      ...
      const { data, error } = await supabase.functions.invoke('detailed-report', {
        body: {
          insightId: id,
          originalInput: foundInsight.input || foundInsight.originalInput, // Fix property name mismatch
          basicInsights: foundInsight.response || foundInsight.insights, // Fix property name mismatch
          generateResources: true,
          includeAffiliateLinks: true
        }
      });

      console.log('Detailed report response:', { data, error });
    ...
    const result = await handleAsyncError(async () => {
      ...
      const { data, error } = await supabase.functions.invoke('email-analysis', {
        body: {
          email,
          analysisData: report,
          thoughtContent: baseInsight.input || baseInsight.originalInput, // Fix property name mismatch
...
Original Thought: ${baseInsight.input || baseInsight.originalInput}
    ...
    const result = await handleAsyncError(async () => {
      ...
      const { data, error } = await supabase.functions.invoke('generate-pdf-report', {
        body: {
          insightId,
          reportData: report,
          originalInput: baseInsight.input || baseInsight.originalInput, // Fix property name mismatch
